TITLE Cerebellum Granule Cell Model, CaHVA channel
COMMENT  
...
FUNCTION alp_u(v(mV))(/ms) { LOCAL Q10
	Q10 = 3^((celsius-20(degC))/10(degC))
if((v-V0alpha_u)/Kalpha_u>200){
 alp_u = Q10*Aalpha_u*exp(200)
}else{
...
FUNCTION bet_u(v(mV))(/ms) { LOCAL Q10
	Q10 = 3^((celsius-20(degC))/10(degC))
if((v-V0beta_u)/Kbeta_u>200){
bet_u = Q10*Abeta_u*exp(200)
}else{
